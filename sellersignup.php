...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($seller_name)) { array_push($errors, "Seller Name is required"); }
  ...
  $user_check_query = "SELECT * FROM seller WHERE username='$username' OR email='$email' OR phone='$phone' OR seller_name='$seller_name' LIMIT 1";
  ...
  if ($user) { // if user exists
    ...
    if ($user['seller_name'] === $seller_name) {
      array_push($errors, "Seller name already exists");
    }
  ...
  if(!empty($filename)){
    $query = "INSERT INTO seller (unique_id, username, seller_name, phone, email, address, image,  password, status) 
          VALUES({$ran_id}, '$username', '$seller_name', '$phone', '$email', '$address', './images/$filename', '$password', '{$status}')";
...
if (move_uploaded_file($tempname, $folder))  {
    ...
    $to_email = $email;
     ...
     $query = "INSERT INTO seller (unique_id, username, seller_name, phone, email, address, image,  password, status) 
          VALUES({$ran_id}, '$username', '$seller_name', '$phone', '$email', '$address', './images/user_profile.png', '$password', '{$status}')";
    mysqli_query($db, $query);
    $_SESSION['username'] = $username;
    $_SESSION['seller_name'] = $seller_name;
    $_SESSION['phone'] = $phone;
...
<body>
  ...
  <form method="post" action="sellersignup.php"  enctype="multipart/form-data">
    ...
    <div class="input-group">
      ...
      <input type="text" name="username" pattern="[^' ']+"  placeholder="name123" required>
      <span class="help-block">(With no blank space)</span>
    </div>
    <div class="input-group">
      <label><b>Seller Name</b></label>
      <input type="text" name="seller_name" placeholder="name Kitchen" required>
    </div>
    <div class="input-group">
    ...
    <div class="input-group">
      ...
      <input type="text" name="address" placeholder="Rawalpindi, Street no 3" required>
    </div> 
    <div class="input-group">
